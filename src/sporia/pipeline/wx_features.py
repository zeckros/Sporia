...
def features_from_daily(daily: dict) -> dict | None:
    ...
    return {
        "rain7": _nansum(pr, 7),
        "rain14": _nansum(pr, 14),
        "rain21": _nansum(pr, 21),
        "rain30": _nansum(pr, 30),
        "rain60": _nansum(pr, 60),
        "tmean14": _nanmean(tm, 14),
        "tamp7": _nanmean(amp, 7),
        "tdrop": tdrop,
        "wbal14": _nansum(pr, 14) - _nansum(et0, 14),
        "wbal30": _nansum(pr, 30) - _nansum(et0, 30),
        "sm_mean": _nanmean(sm0, 7),
        "sm28_mean": _nanmean(sm28, 7),
        "days_since_rain": dsr,
    }
