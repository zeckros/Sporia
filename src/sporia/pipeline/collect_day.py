...
def paquet_json_to_df(arr):
    ...
    for rec in arr:
        ...
        t = rec.get("t")
        rr1 = rec.get("rr1")
        rows.append({"geo_id_insee": geo, "latitude": lat, "longitude": lon, "t": t, "rr1": rr1})
...
def filter_coverages(coverage_ids, day):
    ...
    for cid in coverage_ids:
        ...
        if dt.date() == day:
            if (
                "TEMPERATURE__GROUND_OR_WATER_SURFACE" in cid
                and "BRIGHTNESS_TEMPERATURE" not in cid
            ):
...
def download_coverage(coverage_id, bbox=None):
    ...
    encoded_time = (
        extract_time_from_coverageid(coverage_id).strftime("%Y-%m-%dT%H:%M:%SZ").replace(":", "%3A")
    )
...
def sample_arome_at_points_optimized(arome_filepath, points_lonlat):
    ...
    try:
        ...
        interp = RegularGridInterpolator(
            (ys, xs), vals, method="linear", bounds_error=False, fill_value=np.nan
        )
...
def fetch_arome_for_day(day, points_lonlat, max_hours=24):
    ...
    if cache_file.exists():
        ...
        newer_sources = [
            f
            for f in AROME_DIR.glob("*.tif")
            if any(v in f.stem for v in day_str_variants) and f.stat().st_mtime > cache_mtime
        ]
        if newer_sources:
            logging.info(
                f"AROME source files updated — invalidating NPZ cache ({len(newer_sources)} newer files)"  # noqa: E501
            )
    ...
    if cache_file.exists():
        try:
            ...
            if (
                np.array_equal(cached["points"], points_lonlat)
    ...
    if not temp_ids and not precip_ids:
        ...
        logging.warning(
            f"[AROME DIAG] Aucun ID pour {day}. Dates disponibles dans l'API: {sorted(dates_dispo)}"
        )
        return pd.DataFrame()
    ...
    try:
        np.savez_compressed(cache_file, points=points_lonlat, precip=precip_vals, temp=temp_mean)
...
def main():
    ...
    if radar_files:
        logging.info(
            f"✓ {len(radar_files)} fichier(s) radar extrait(s): {[f.name for f in radar_files]}"
        )
    else:
    ...
    if not arome_csv_today.exists():
        ...
        logging.info("[AROME] CSV absent → fetch déclenché")
    else:
        csv_age_h = (
            now - datetime.fromtimestamp(arome_csv_today.stat().st_mtime, tz=TZ)
        ).total_seconds() / 3600
        should_fetch_arome = csv_age_h >= 6
        logging.info(
            f"[AROME] CSV présent, âge={csv_age_h:.1f}h → fetch={'oui' if should_fetch_arome else 'non'}"  # noqa: E501
        )
