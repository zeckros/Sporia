...
def hierarchical_select(df):
    ...
    et pour each unique (latitude,longitude) keep the row with highest priority
...
def interpret_day(day: date):
    ...
    if stations_file.exists():
        try:
            ...
            if rr_col:
                agg_spec["RR"] = (
                    rr_col,
                    lambda x: pd.to_numeric(x, errors="coerce").sum(min_count=1),
                )
            if t_col:
    ...
    if pts_a is not None:
        rr_arome_grid = idw(pts_a, df_a["RR"].values.astype(float), grid_pts, power=2, k=8).astype(
            np.float32
        )
        t_arome_grid = idw(pts_a, df_a["T"].values.astype(float), grid_pts, power=2, k=8).astype(
            np.float32
        )
    ...
    if da_radar is not None:
        try:
            ...
            if lons_r[0] > lons_r[-1]:
                lons_r = lons_r[::-1]
                vals_r = vals_r[:, ::-1]
            _radar_lats, _radar_lons = lats_r, lons_r
            ...
            logging.info(
                f"Radar rééchantillonné : {np.isfinite(rr_radar_grid).sum()} cellules valides"
            )
        except Exception as e:
            ...
            if valid_bias.sum() >= 2:
                ...
                if len(biases) >= 4:
                    ...
                    if keep.sum() >= 2:
                        ...
                        biases = biases[keep]
                bias_grid = idw(pts_bias, biases, grid_pts, power=2, k=min(8, len(biases))).astype(
                    np.float32
                )
    ...
    if pts_s is not None:
        ...
        if pts_a is not None:
            ...
            if valid_t.sum() >= 2:
                ...
                res_grid = idw(
                    pts_s[valid_t], residuals, grid_pts, power=2, k=min(8, int(valid_t.sum()))
                ).astype(np.float32)
                t_final = t_arome_grid + res_grid
                logging.info(
                    f"Résidus T : {valid_t.sum()} stations, résidu moyen={residuals.mean():.2f} K"
                )
        else:
...
def stack_days_and_compute(days, var="RR", agg="sum"):
    ...
    out_name = (
        OUT_TIF_DIR
        / f"{var}_{days[0].strftime('%Y%m%d')}_to_{days[-1].strftime('%Y%m%d')}_{agg}.tif"
    )
