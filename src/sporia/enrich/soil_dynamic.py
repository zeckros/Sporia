...
def _fetch_batch(pts_lonlat: np.ndarray) -> list[tuple[float, float, float, float]]:
    ...
    out = []
    for p, loc in zip(pts_lonlat, locs, strict=False):
...
def fetch_soil_grid() -> np.ndarray | None:
    ...
    for i in range(0, len(pts), _BATCH):
        ...
        logging.info(f"[soil] batch {i // _BATCH + 1}/{n_batches} ({len(batch)} pts)")
...
def produce_soil_rasters(day: date, force: bool = False):
    ...
    try:
        np.savetxt(
            DAILY_DIR / f"soil_{day.strftime('%Y%m%d')}.csv",
            data,
            delimiter=",",
            header="lon,lat,soil_moisture,soil_temp",
            comments="",
            fmt="%.5f",
        )
    except Exception:
    ...
    logging.info(
        f"[soil] wrote {out_sm.name} (SM {np.nanmean(sm):.3f} m³/m³) "
