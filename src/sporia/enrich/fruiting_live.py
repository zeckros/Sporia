...
def _static_layers():
    ...
    layers = {
        ...
        "sand": soil["sand"],
        "silt": soil["silt"],
        "altitude": terr["altitude"],
        "slope": terr["slope"],
        "northness": terr["northness"],
    }
...
def _fetch_recent_points(step=0.3, batch=150):
    ...
    for i in range(0, len(pts), batch):
        ...
        for pt, rec in zip(chunk, recs, strict=False):
...
def score_species(species: str, date_str: str | None = None):
    ...
    layers = {**stat, **temp}
    cols = np.stack(
        [layers[f].ravel() for f in feats], axis=1
    )  # (N, n_feats) dans l'ordre du modèle
...
def blended_at_point(
    species: str, lat: float, lon: float, date_str: str | None = None, params: dict | None = None
):
...
def prewarm(date_str: str | None = None):
    ...
    t0 = time.time()
    grids = recent_temporal_grid(
        date_str, allow_fetch=True
    )  # seul appelant qui interroge Open-Meteo
    if grids is None:
...
if __name__ == "__main__":
    ...
    if g is None:
        ...
        print(f"{sp} @ {d} : {int(np.isfinite(g).sum())} cellules, max {np.nanmax(g):.2f}")
