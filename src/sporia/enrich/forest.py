...
def forest_at_point(lat: float, lon: float) -> dict | None:
    ...
    try:
        with urllib.request.urlopen(
            url, timeout=5
        ) as r:  # court : ne pas bloquer un thread si l'IGN traîne
...
def _family_from_strings(tfv_g11: str, essence: str, tfv: str) -> str:
    ...
    if "mixte" in s or ("feuillus" in s and ("conif" in s or "résineux" in s)):
        return "mixte"
    if "conif" in s or any(
        k in s
        for k in ("pin", "sapin", "épicéa", "epicea", "douglas", "mélèze", "meleze", "résineux")
    ):
        return "coniferes"
    if "feuillus" in s or any(
        k in s
        for k in (
            "chêne",
            "chene",
...
def family_at_point(lat: float, lon: float) -> dict | None:
    ...
    b = (
        float(layers["feuillus"][row, col])
        if "feuillus" in layers and np.isfinite(layers["feuillus"][row, col])
        else 0.0
    )
    n = (
        float(layers["coniferes"][row, col])
        if "coniferes" in layers and np.isfinite(layers["coniferes"][row, col])
        else 0.0
    )
    m = (
        float(layers["mixte"][row, col])
        if "mixte" in layers and np.isfinite(layers["mixte"][row, col])
        else 0.0
    )
    tot = b + n + m
...
def _load_windows(data_dir: Path, ref_date, available_dates, rain_windows=(7, 14), temp_days=14):
...
def _eff_temp(temp_air, soil_temp):
    ...
    return np.where(
        np.isnan(soil_temp),
        temp_air,
        np.where(np.isnan(temp_air), soil_temp, 0.5 * temp_air + 0.5 * soil_temp),
    )
