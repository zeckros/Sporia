...
def _slope_northness(alt: np.ndarray):
    ...
    z = np.where(
        np.isnan(alt), np.nanmean(alt[np.isfinite(alt)]) if np.isfinite(alt).any() else 0.0, alt
    )
...
def sample_terrain_static(lat: float, lon: float) -> dict | None:
    ...
    return {
        "altitude": None if not np.isfinite(alt) else round(float(alt), 0),
        "slope_pct": round(slope, 1),
        "exposition": expo,
        "northness": round(north, 2),
        "source": "grille (Open-Meteo)",
    }
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "bake":
        ...
        v = a[np.isfinite(a)]
        print(
            f"  altitude  shape={a.shape} min={v.min():.0f} mean={v.mean():.0f} max={v.max():.0f} m"
        )
        print(f"  slope     mean={np.nanmean(g['slope']):.1f}% max={np.nanmax(g['slope']):.0f}%")
        print(
            f"  northness range=({np.nanmin(g['northness']):.2f}..{np.nanmax(g['northness']):.2f})"
        )
    else:
